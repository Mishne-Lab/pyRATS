    ...
    if beta is None:
        ...
        if T is not None and v is not None:
            return (
                np.dot(
                    beta[k]
                    * Psi_gamma[k, :][np.newaxis, :]
                    * phi[np.ix_(mask, Psi_i[k, :])],
                    T[k, :, :],
                )
                + v[[k], :]
            )
...
def colorcube(m):
    ...
    summap = np.concatenate(
        [
            np.sum(mymap[:, [0, 1]], 1)[:, np.newaxis],
            np.sum(mymap[:, [1, 2]], 1)[:, np.newaxis],
            np.sum(mymap[:, [0, 2]], 1)[:, np.newaxis],
        ],
        axis=1,
    )
...
class Visualize:
    def __init__(self, save_dir=""):
        ...
        if self.save_dir:
            if (not os.path.isdir(self.save_dir)) and (
                not os.path.islink(self.save_dir)
            ):
    ...
    def init_3d_subplots(self, n_row, n_col, subplot_h=1, subplot_w=1):
        fig, ax = plt.subplots(
            n_row,
            n_col,
            figsize=(n_col * subplot_w, n_row * subplot_h),
    ...
    def data(
        ...
        s_focus_scale=5,
        n_c_bar_ticks=4,
        make_label_mask=False,
        vmin=None,
        vmax=None,
    ):
        if type(X) == list:
            X = np.concatenate(
                [X[0].flatten()[:, None], X[1].flatten()[:, None]], axis=1
            )
        if labels is None:
        ...
        if X.shape[1] == 2:
            plt.scatter(
                X[:, 0], X[:, 1], s=s, c=labels, cmap=cmap, vmin=vmin, vmax=vmax
            )
            if focus_pts is not None:
                plt.scatter(
                    X[focus_pts, 0], X[focus_pts, 1], s=s_focus_scale * s, c="r"
                )
            if axis_image:
                plt.axis("image")
            if colorbar:
            ...
            if colorbar:
                cbar_ticks = np.linspace(
                    np.nanmin(labels), np.nanmax(labels), n_c_bar_ticks, endpoint=True
                )
                cbar = fig.colorbar(p)
    ...
    def grad_phi(self, X, phi, grad_phi, i, prop=0.01, figsize=None, s=20):
        ...
        if X.shape[1] == 2:
            ...
            plt.subplot(122)
            plt.quiver(
                X[mask, 0], X[mask, 1], grad_phi[mask, i, 0], grad_phi[mask, i, 1]
            )
            plt.axis("image")
            plt.title(r"$\nabla\phi_{%d}$" % i)
        elif X.shape[1] == 3:
    ...
    def Atilde(self, X, phi, i, j, Atilde, figsize=None, s=20):
        ...
        if X.shape[1] == 2:
            ...
            plt.axis("image")
            plt.colorbar()
            plt.title(r"$\phi_{%d}$" % j)
            plt.subplot(133)
            plt.scatter(X[:, 0], X[:, 1], s=s, c=Atilde[:, i, j], cmap="jet")
            plt.axis("image")
            plt.colorbar()
            plt.title(r"$\widetilde{A}_{:%d%d}$" % (i, j))
        elif X.shape[1] == 3:
            ...
            p = ax.scatter(
                X[:, 0], X[:, 1], X[:, 2], s=s, c=Atilde[:, i, j], cmap="jet"
            )
            set_axes_equal(ax)
            fig.colorbar(p, ax=ax)
            plt.title(r"$\widetilde{A}_{:%d%d}$" % (i, j))
    ...
    def n_eigvecs_w_grad_lt(
        self, X, Atilde, thresh_prctile=None, figsize=(16, 8), s=20
    ):
        assert X.shape[1] <= 3, "X.shape[1] must be either 2 or 3."
        fig = plt.figure(figsize=figsize)
        if matplotlib.get_backend().startswith("Qt"):
        ...
        while True:
            ...
            plt.xlabel("percentiles")
            plt.title(
                r"$\\widetilde{A}_{kii}$, threshold = %f\nDouble click = Select threshold\nPress button = quit"
                % thresh
            )
            fig.canvas.draw()
            fig.canvas.flush_events()
            ...
            if X.shape[1] == 2:
                ...
                ax.set_title(r"$n_k = \sum_{i}\widetilde{A}_{kii} < %f$" % thresh)
            elif X.shape[1] == 3:
                ax.cla()
                ax.autoscale()
            ...
            if self.save_dir:
                ...
                plt.savefig(
                    self.save_dir + "/n_eigvecs_w_grad_lt/" + str(thresh) + OUTPUT_EXT
                )
            plt.show()
    ...
    def global_distortion_viloinplot_overlay(
        self,
        dist_dicts,
        label=r"$\log(\mathcal{G}_k)$",
        ...
        showmeans=False,
        showextrema=False,
        plotlinethrghmedians=False,
        plotlinethrghmeans=False,
        stat_color="red",
    ):
        lw = 1
        ...
        if legend:
            custom_lines = [
                Line2D([0], [0], color=color[k], lw=lw, alpha=1) for k in color.keys()
            ]
    ...
    def global_distortion_viloinplot(
        self,
        dist_dict,
        ylabel=r"$\log(D_k)$",
        title=r"violinplot for $\log(D_k)$",
        log_scale=True,
        figsize=None,
    ):
        fig = plt.figure(figsize=figsize)
        if log_scale:
            ...
            plt.ylabel(r"$D_k$")
            plt.title(r"Violinplot for $D_k$")
        if self.save_dir:
            plt.savefig(
                self.save_dir + "/global_distortion_violinplot.eps", format="eps"
            )
        plt.show()
    ...
    def chosen_eigevec_inds_for_intermediate_views(
        ...
        if X.shape[1] == 2:
            ...
            ax = fig.add_subplot(121, projection="3d")
            p = ax.scatter(
                X[:, 0], X[:, 1], X[:, 2], s=s, c=Psitilde_i[c, 0], cmap="jet"
            )
            set_axes_equal(ax)
            fig.colorbar(p)
            plt.title(r"$\\phi_{i_1}$")
            ax = fig.add_subplot(122, projection="3d")
            p = ax.scatter(
                X[:, 0], X[:, 1], X[:, 2], s=s, c=Psitilde_i[c, 1], cmap="jet"
            )
            set_axes_equal(ax)
            fig.colorbar(p)
            ax.set_title(r"$\\phi_{i_2}$")
    ...
    def local_views(
        ...
        while True:
            ...
            if is_3d_data:
                p = ax[0].scatter(X[:, 0], X[:, 1], X[:, 2], s=s, c=zeta, cmap="jet")
                ax[0].scatter(
                    X[:, 0], X[:, 1], X[:, 2], s=s, c=combine_cmaps(zeta, U_k)
                )
                set_axes_equal(ax[0])
            ...
            cb[0] = plt.colorbar(p, ax=ax[0])
            ax[0].set_title(r"$\\mathcal{M}$ and $U_{%d}$" % k)

            # Plot the corresponding local view in the embedding space
            y = local_param.eval_({"view_index": k, "data_mask": np.arange(n)})
            ax[3]
            ax[3].cla()
            ...
            for j in range(len(subplots)):
                ...
                if is_3d_data:
                    ...
                    ax[j + 1].scatter(
                        ...
                        X[:, 2],
                        s=s,
                        c=combine_cmaps(local_param.phi[:n, i_s], U_k),
                    )
            ...
            ax[4].plot(
                [0, 100],
                [angles[local_param.Psi_i[k, 0], local_param.Psi_i[k, 1]]] * 2,
                "r-",
            )
            ax[4].set_xlabel("percentiles")
            ax[4].set_title(r"$\\cos(\\nabla\\phi_{i_1},\\nabla\\phi_{i_2})$")
            ...
            ax[5].cla()
            ax[5].plot(
                prctiles, np.percentile(dlocal_scales.flatten(), prctiles), "bo-"
            )
            ax[5].plot([0, 100], [np.log(2)] * 2, "g-")
            ax[5].plot(
                [0, 100],
                [dlocal_scales[local_param.Psi_i[k, 0], local_param.Psi_i[k, 1]]] * 2,
                "r-",
            )
            ax[5].set_xlabel("percentiles")
            ax[5].set_title(
                r"$\\log(\\gamma_{i_1}\\left\\|\\nabla\\phi_{i_1}\\right\\|_2 / \
    ...
    def local_views_lpca(
        self, X, local_param, U, k=None, save_subdir="", figsize=(15, 10), s=20
    ):
        assert X.shape[1] <= 3, "X.shape[1] must be either 2 or 3."
        is_3d_data = X.shape[1] == 3
        n = U.shape[0]
        ...
        while True:
            ...
            if is_3d_data:
                p = ax[0].scatter(
                    X[:, 0], X[:, 1], X[:, 2], s=s * (1 - U_k), c=zeta, cmap="jet"
                )
            ...
            for j in range(len(subplots)):
                ...
                if is_3d_data:
                    ...
                    V = local_param.Psi[:, :, j]
                    p = ax[j + 2].quiver(
                        X[:, 0], X[:, 1], V[:, 0], V[:, 1], color="r", linewidths=0.1
                    )
                    ax[j + 2].quiver(
                        X[U_k, 0],
                        X[U_k, 1],
    ...
    def intermediate_views(
        ...
        ax[0].set_title(
            "Double click = select an intermediate view.\n Press button = exit."
        )
        cb[0] = plt.colorbar(p, ax=ax[0])
        ...
        while True:
            ...
            if is_3d_data:
                p = ax[0].scatter(X[:, 0], X[:, 1], X[:, 2], s=s, c=zeta, cmap="jet")
                ax[0].scatter(
                    X[:, 0], X[:, 1], X[:, 2], s=s, c=combine_cmaps(zeta, Utilde_m)
                )
                set_axes_equal(ax[0])
            else:
                p = ax[0].scatter(
                    X[:, 0], X[:, 1], s=s * (1 - Utilde_m), c=zeta, cmap="jet"
                )
                ax[0].scatter(X[Utilde_m, 0], X[Utilde_m, 1], s=s, c="k")
                ax[0].axis("image")

            cb[0] = plt.colorbar(p, ax=ax[0])
            ax[0].set_title(r"$\\mathcal{M}$ and $\\widetilde{U}_{%d}$" % m)
            ...
            ax[3].set_title(
                r"Distortion=%.3f\n"
                r" $\\widetilde{\\Phi}_{%d}(\\mathcal{M})$ in red\n"
                r" and $\\widetilde{\\Phi}_{%d}(\\widetilde{U}_{%d})$ in black"
                % (zetatilde[m], m, m, m)
            )  # zetatilde[m] == zeta[k]
            ...
            for j in range(len(subplots)):
                ...
                if is_3d_data:
                    p = ax[j + 1].scatter(
                        X[:, 0], X[:, 1], X[:, 2], s=s, c=phi[:, i_s], cmap="jet"
                    )
                    ax[j + 1].scatter(
                        X[:, 0],
                        X[:, 1],
                        X[:, 2],
                        s=s,
                        c=combine_cmaps(phi[:, i_s], Utilde_m),
                    )
                    set_axes_equal(ax[j + 1])
                else:
                    p = ax[j + 1].scatter(
                ...
                cb[j + 1] = plt.colorbar(p, ax=ax[j + 1])
                ax[j + 1].set_title(r"$\\widetilde{\\phi}_{%d}$" % i_s)
            ...
            ax[4].set_xlabel("percentiles")
            ax[4].set_title(
                r"$|\\widetilde{A}_{%dij}|/(\\widetilde{A}_{%dii}\\widetilde{A}_{%djj})$"
                % (k, k, k)
            )
            ...
            if self.save_dir:
                if not os.path.isdir(
                    self.save_dir + "/intermediate_views/" + save_subdir
                ):
    ...
    def compare_local_high_low_distortion(
        self, X, Atilde, Psi_gamma, Psi_i, zeta, save_subdir="", figsize=None, s=20
    ):
        assert X.shape[1] <= 3, "X.shape[1] must be either 2 or 3."
        is_3d_data = X.shape[1] == 3
        ...
        ax[0].set_title(r"$x_k$ colored by $\\zeta_{kk}$")
        ax[3].axis("off")
        ...
        while True:
            ...
            ax[4].cla()
            ax[4].boxplot(
                [angles[low_dist_mask], angles[high_dist_mask]],
                labels=[r"low $\\zeta_{kk}$", r"high $\\zeta_{kk}$"],
                notch=True,
                vert=False,
                patch_artist=True,
            )
            ax[4].set_title(
                r"$|\\widetilde{A}_{ki_1i_2}|/(\\widetilde{A}_{ki_1i_1}\\widetilde{A}_{ki_2i_2})$"
            )

            ax[5]
            ax[5].cla()
            ax[5].boxplot(
                [dlocal_scales[low_dist_mask], dlocal_scales[high_dist_mask]],
                labels=[r"low $\\zeta_{kk}$", r"high $\\zeta_{kk}$"],
                notch=True,
                vert=False,
                patch_artist=True,
            ...
            if self.save_dir:
                ...
                plt.savefig(
                    self.save_dir
                    + "/local_high_low_distortion/thresh="
                    + str(thresh)
                    + OUTPUT_EXT
                )
            plt.show()

    def compare_intermediate_high_low_distortion(
        self,
        X,
        Atilde,
        Psitilde_gamma,
        ...
        Atilde_ki_1i_2 = np.abs(
            Atilde[np.arange(M), Psitilde_i[:, 0], Psitilde_i[:, 1]]
        )
        Atilde_ki_1i_1 = np.sqrt(
            Atilde[np.arange(M), Psitilde_i[:, 0], Psitilde_i[:, 0]]
        )
        Atilde_ki_2i_2 = np.sqrt(
            Atilde[np.arange(M), Psitilde_i[:, 1], Psitilde_i[:, 1]]
        )
        angles = (Atilde_ki_1i_2 / Atilde_ki_1i_1) / (Atilde_ki_2i_2.T)
        ...
        cb = plt.colorbar(p, ax=ax[0])
        ax[0].set_title(r"$x_k$ colored by $\\widetilde{\\zeta}_{c_kc_k}$")
        ax[3].axis("off")
        ...
        while True:
            ...
            ax[1].set_xlabel("percentiles")
            ax[1].set_title(
                r"$\\widetilde{\\zeta}_{mm}$, threshold = %f\nDouble click = Select threshold\nPress button = quit"
                % thresh
            )
            fig.canvas.draw()
            fig.canvas.flush_events()
            ...
            ax[4].boxplot(
                [angles[low_dist_mask], angles[high_dist_mask]],
                labels=[
                    r"low $\\widetilde{\\zeta}_{mm}$",
                    r"high $\\widetilde{\\zeta}_{mm}$",
                ],
                notch=True,
                vert=False,
                patch_artist=True,
            ...
            if self.save_dir:
                if not os.path.isdir(
                    self.save_dir + "/intermediate_high_low_distortion"
                ):
                    os.makedirs(self.save_dir + "/intermediate_high_low_distortion")
                plt.savefig(
                    self.save_dir
                    + "/intermediate_high_low_distortion/thresh="
                    + str(thresh)
                    + OUTPUT_EXT
                )
            plt.show()

    def seq_of_intermediate_views(
        self, X, c, seq, rho, Utilde, figsize=None, s=20, cmap="jet"
    ):
        seq = np.copy(seq)
        M = seq.shape[0]
    ...
    def global_embedding(
        ...
        if d == 2:
            if ax is None:
                ax = fig.add_subplot()
            ax.scatter(
                y[:, 0], y[:, 1], s=s, c=labels, cmap=cmap0, vmin=vmin, vmax=vmax
            )
            ax.axis("image")
        elif d == 3:
            if ax is None:
        ...
        if self.save_dir:
            ...
            plt.savefig(
                self.save_dir + "/ge/" + str(title) + OUTPUT_EXT, bbox_inches="tight"
            )
        # plt.show()
        return ax

    def global_embedding_for_gui(
        self,
        y,
        labels,
        cmap0,
        ...
        if color_of_pts_on_tear is not None:
            ...
            if cmap1 == "colorcube" and (len(color_of_pts_on_tear.shape) == 1):
                ...
                if d == 2:
                    pts = ax.scatter(y[:, 0], y[:, 1], s=s, c=c_)
                elif d == 3:
                    pts = ax.scatter(y[:, 0], y[:, 1], y[:, 2], s=s, c=c_)
                    set_axes_equal(ax)
            else:
                if d == 2:
                    pts_boundary = ax.scatter(
                        y[pts_on_tear, 0],
                        y[pts_on_tear, 1],
                        s=s,
                        c=color_of_pts_on_tear[pts_on_tear],
        ...
        if self.save_dir:
            if not os.path.isdir(self.save_dir + "/ge"):
                os.makedirs(self.save_dir + "/ge")
            plt.savefig(
                self.save_dir + "/ge/" + str(title) + OUTPUT_EXT,
                bbox_inches="tight",
                pad_inches=0,
            )
        # plt.show()
        return pts, pts_boundary

    def global_embedding_images(
        self,
        X,
        img_shape,
        y,
        ...
        ax.axis("image")
        if color_of_pts_on_tear is not None:
            ax.scatter(y[:, 0], y[:, 1], s=s, c=color_of_pts_on_tear, cmap=cmap1)
    ...
    def global_embedding_images_v2(
        self,
        X,
        img_shape,
        y,
        ...
        pts = np.concatenate(
            [
                np.concatenate(
                    [x_i[:, np.newaxis], np.zeros((nx, 1)) + y_y_min], axis=1
                ),
                np.concatenate(
                    [x_i[:, np.newaxis], np.zeros((nx, 1)) + y_y_max], axis=1
                ),
                np.concatenate(
                    [np.zeros((ny, 1)) + y_x_min, y_i[:, np.newaxis]], axis=1
                ),
                np.concatenate(
                    [np.zeros((ny, 1)) + y_x_max, y_i[:, np.newaxis]], axis=1
                ),
            ],
            axis=0,
        )
        ...
        if color_of_pts_on_tear is not None:
            if len(color_of_pts_on_tear.shape) > 1:
                mask = ~np.isnan(color_of_pts_on_tear[:, 0])
            else:
                mask = ~np.isnan(color_of_pts_on_tear)
            ax.scatter(
                y[mask, 0], y[mask, 1], s=s, c=color_of_pts_on_tear[mask], cmap=cmap1
            )
        # ax.set_title(title)
        ...
        for k_ in range(pts.shape[0]):
            ...
            if to_T:
                imscatter(
                    pts[k_, 0],
                    pts[k_, 1],
                    X[k, :].reshape(img_shape).T,
                    ax=ax,
                    zoom=zoom,
                )
            else:
                imscatter(
                    pts[k_, 0], pts[k_, 1], X[k, :].reshape(img_shape), ax=ax, zoom=zoom
                )
            ax.quiver(
                y[k, 0],
                y[k, 1],
                v_ratio * (pts[k_, 0] - y[k, 0]),
                v_ratio * (pts[k_, 1] - y[k, 1]),
                units="xy",
                scale=1,
                width=w_ratio * x_width,
            )
    ...
    def visualize_epoch_data(self, X, Ls):
        ...
        for i in range(len(Ls)):
            ...
            plt.scatter(
                x1[b - 1], y1[b - 1], color="white", marker="o", edgecolor="black"
            )
            plt.axis("image")

    def visualize_intermediate_views(
        X, c, Utilde, figsize=(5, 5), s=1, subsample_inds=[], view_color="r", alpha=0.25
    ):
        plt.figure(figsize=figsize)
        plt.scatter(*X.T, c=c, s=s)
        for k in subsample_inds:
            plt.scatter(*X[Utilde[k, :], :].T, c=view_color, alpha=alpha)
