...
class CutAndPaste:
    ...
    def get_current_embedding_data(self):
        ...
        source_data_dict = dict(
            x=y[:, 0], y=y[:, 1], color=rgba_arr_to_css_arr(y_face_color)
        )
    ...
    def cut_clusters_to_move(self, selected_pts_indices):
        ...
        if self.force_compute or (
            self.metadata[self.cur_iter]["selected_cluster_mask"] is None
        ):
            selected_cluster_mask = self.select_clusters_to_move(selected_pts_indices)
            self.metadata[self.cur_iter][
                "selected_cluster_mask"
            ] = selected_cluster_mask
        else:
            selected_cluster_mask = self.metadata[self.cur_iter][
                "selected_cluster_mask"
            ]
    ...
    def paste_clusters(self, final_polyg):
        if self.force_compute or (
            self.metadata[self.cur_iter]["final_polygon"] is None
        ):
    ...
    def finish_pasting(self, y_new):
        ...
        self.y, model.Utildeg, _ = compute_final_embedding(
            y_new,
            model.d,
            model.Utilde,
            model.C,
            ...
            model.seq_of_intermed_views_in_cluster,
            model.verbose,
        )
    ...
    def recompute_embedding(
        self, y, points_in_moving_clusters, init_polyg, final_polyg
    ):
        ...
        y = y.copy()
        y[points_in_moving_clusters, :] = (
            y[points_in_moving_clusters, :].dot(O) + t[None, :]
        )
        return y
...
print_main_log(
    "<h3>Short logs & instructions are displayed here. Detiled logs are printed at the end of the page.</h3>"
)
...
update_max_fig_height_button = Button(
    label="Update height", button_type="success", disabled=True
)
...
rotate_cw_button = Button(
    label="Rotate -" + str(rot_deg) + " deg", button_type="success", disabled=True
)
rotate_ccw_button = Button(
    label="Rotate +" + str(rot_deg) + " deg", button_type="success", disabled=True
)
...
fig.scatter("x", "y", color="color", source=source, nonselection_alpha=0.3)
...
def on_pan(event):
    ...
    x_out_of_bounds = np.any(new_x < fig.x_range.bounds[0]) | np.any(
        new_x > fig.x_range.bounds[1]
    )
    y_out_of_bounds = np.any(new_y < fig.y_range.bounds[0]) | np.any(
        new_y > fig.y_range.bounds[1]
    )
    if not (x_out_of_bounds or y_out_of_bounds):
        patch_source.data.update(x=(new_x).tolist(), y=(new_y).tolist())
    dragging["start_x"] = event.x
    dragging["start_y"] = event.y
...
def rotate_coords(x, y, cx, cy, angle):
    ...
    x_out_of_bounds = np.any(x_rot < fig.x_range.bounds[0]) | np.any(
        x_rot > fig.x_range.bounds[1]
    )
    y_out_of_bounds = np.any(y_rot < fig.y_range.bounds[0]) | np.any(
        y_rot > fig.y_range.bounds[1]
    )
...
def on_save_button_click():
    curdoc().add_next_tick_callback(save_)
